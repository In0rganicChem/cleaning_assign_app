...
def distribute_one_week(capacities, prev_week_a_set, rnd):
    ...
    d_groups = [c for c in class_names if capacities[c] >= 7]
    ...
    # -------------------------
    # 3. 그룹 A를 아직 배정받지 못한 4명 이상 생활반에 배정
    ...
    # -------------------------
    # 4. 그룹 B를 아직 배정받지 못한 4명 이상 생활반에 배정
    # -------------------------
    ...
    # -------------------------
    # 5. 그룹 C를 아직 배정받지 못한 4명 이상 생활반에 배정
    # -------------------------
    ...
    # -------------------------
    # 7. 인원이 4명 이상인 생활반 중, 아직 1개만 배정받은 곳에 D 배정
    # -------------------------
...
st.header("생활반 별 청소가능인원을 입력하십시오 (0명 이상)")
...
자세한 내용은 아래 "상세 작동 알고리즘 보기" 참고  
화장실 및 목욕탕은 지나치게 많이 배치받는 생활반이 없도록 수동으로 조정됩니다.
"""
        )
...
for i in range(0, len(class_names), 3):
    ...
    for col, cname in zip(cols, row):
        # max_value 파라미터 없음. 12명 이상 입력 가능.
...
사용자는 다음의 값들을 프로그램에 입력한다.
1. 생활반 별 청소가능인원(0 이상의 정수)
2. 지난 주에 그룹 A의 청소구역(1층 화장실, 2층 화장실, 3층 화장실, 1층 목욕탕)을 배정받은 생활반
3. 난수 시드(정수, 같은 결과를 보장하기 위해 입력.)


<청소구역 배정 알고리즘>

1. 청소가능인원이 0명인 생활반은 어떤 청소구역도 배정받지 않는다.

1-1. 만약 모든 생활반의 청소가능인원이 0명이라면, 이스터에그로 고양이 사진을 보여준다.

1-2. 만약 인원이 0-3명인 생활반이 7개 이상이라면, 이후 이어지는 모든 단계에서 "그룹 A의 청소구역 4개는 2주 연속 배정받는 일이 없어야 한다." 라는 규칙을 무시하고 청소구역을 배정한다.
이때, 마지막 결과창에 "청소가능인원이 매우 부족하여 2주 연속 화장실/목욕탕을 청소하는 생활반이 나올 수 있습니다." 라는 경고창을 전시한다.

2. 청소가능인원이 1-3명인 생활반에 청소구역을 하나씩 랜덤으로 배정한다. 청소구역의 그룹과는 관계 없이, 25개의 구역 중 하나를 랜덤으로 배정한다.

2-1. 지난 주에 그룹 A의 청소구역을 청소한 생활반이 또 그룹 A를 배정받았는지 확인한다. 만약 그렇다면, 2단계를 다시 시행한다. 즉, 그룹 A의 청소구역 4개는 2주 연속 배정받는 일이 없어야 한다.

3. 그룹 A의 청소구역이 아직 남아 있을 것이다.(4개, 만약 2단계에서 청소가능인원이 1-3명인 생활반이 그룹 A를 받았다면 4개 미만)
아직 청소구역을 배정받지 않은 생활반 (청소가능인원이 4명 이상인 생활반)에 랜덤으로 이들을 하나씩 배정한다.

3-1. 지난 주에 그룹 A의 청소구역을 청소한 생활반이 또 그룹 A를 배정받았는지 확인한다. 만약 그렇다면, 3단계를 다시 시행한다. 즉, 그룹 A의 청소구역 4개는 2주 연속 배정받는 일이 없어야 단다.

3-2. 3단계에서, "아직 청소구역을 배정받지 않은 생활반"의 수보다 "남은 그룹 A의 청소구역"의 수가 크다면, 배정되지 못하고 남은 그룹 A의 청소구역이 있을 것이다. 이들은 "인원 부족으로 배정되지 못한 청소구역"으로 분류한다.

4. 그룹 B의 청소구역이 아직 남아 있을 것이다.(2개, 만약 2단계에서 청소가능인원이 1-3명인 생활반이 그룹 B를 받았다면 2개 미만)
아직 청소구역을 배정받지 않은 생활반 (청소가능인원이 4명 이상인 생활반 중 3단계에서 그룹 A를 배정받지 않은 생활반)에 랜덤으로 이들을 하나씩 배정한다.

4-1. 4단계에서, "아직 청소구역을 배정받지 않은 생활반"의 수보다 "남은 그룹 B의 청소구역"의 수가 크다면, 배정되지 못하고 남은 그룹 B의 청소구역이 있을 것이다. 이들은 "인원 부족으로 배정되지 못한 청소구역"으로 분류한다.

5. 그룹 C의 청소구역이 아직 남아 있을 것이다.(5개, 만약 2단계에서 청소가능인원이 1-3명인 생활반이 그룹 C를 받았다면 5개 미만)
아직 청소구역을 배정받지 않은 생활반 (청소가능인원이 4명 이상인 생활반 중 2, 3단계에서 그룹 A, B를 배정받지 않은 생활반)에 랜덤으로 이들을 하나씩 배정한다.

6. 5단계에서 아직 배정되지 않고 남은 그룹 C의 청소구역의 수를 n이라고 하자. 가장 인원이 많은 생활반 n개를 뽑는다.(인원이 같을 경우 랜덤추출) 이 n개의 생활반에 남은 그룹 C의 청소구역을 랜덤으로 분배한다.

7. 인원이 4명 이상인 생활반 중, 아직 청소구역을 하나만 배정받은 생활반에 그룹 D의 청소구역을 하나씩 랜덤으로 배정한다.

8. 그룹 D의 청소구역이 아직 남아 있을 것이다. 그 수를 n이라고 하자. 가장 인원이 많은 생활반 n개를 뽑는다.(인원이 같을 경우 랜덤추출) 이 n개의 생활반에 남은 그룹 D의 청소구역을 랜덤으로 하나씩 분배한다.

8-1. 8단계에서, "인원이 0명이 아닌 생활반"의 수보다 "남은 그룹 D의 청소구역"의 수(n)가 크다면, 배정되지 못하고 남은 그룹 D의 청소구역이 있을 것이다. 이들은 "인원 부족으로 배정되지 못한 청소구역"으로 분류한다.

9. 1-8단계를 총 5번 반복하여 5주차의 청소구역을 배정한다.

10. 대표수병이 수동으로 밸런스를 조정한다.
"""
)
    except Exception as e:
        st.error(str(e))
