...
아직 청소구역을 배정받지 않은 생활반 (청소가능인원이 4~11명인 생활반)에 랜덤으로 이들을 하나씩 배정한다.

3-1. 지난 주에 그룹 A의 청소구역을 청소한 생활반이 또 그룹 A를 배정받았는지 확인한다. 만약 그렇다면, 3단계를 다시 시행한다. 즉, 그룹 A의 청소구역 4개는 2주 연속 배정받는 일이 없어야 한다.
...
아직 청소구역을 배정받지 않은 생활반 (청소가능인원이 4~11명인 생활반 중 3단계에서 그룹 A를 배정받지 않은 생활반)에 랜덤으로 이들을 하나씩 배정한다.
...
아직 청소구역을 배정받지 않은 생활반 (청소가능인원이 4~11명인 생활반 중 2, 3단계에서 그룹 A, B를 배정받지 않은 생활반)에 랜덤으로 이들을 하나씩 배정한다.

6. 5단계에서 아직 배정되지 않고 남은 그룹 C의 청소구역의 수를 n이라고 하자. 가장 인원이 많은 생활반 n개를 뽑는다.(인원이 같을 경우 랜덤추출) 이 n개의 생활반에 남은 그룹 C의 청소구역을 랜덤으로 분배한다.

7. 인원이 4-11명인 생활반 중, 아직 청소구역을 하나만 배정받은 생활반에 그룹 D의 청소구역을 하나씩 랜덤으로 배정한다.
